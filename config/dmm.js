...
const config = {
  entry: {
    url: "https://www.dmm.co.jp/mono/dvd/-/list/=/sort=ranking/"
  },
  name: "dmm",
  options,
  rule: [
    ...
    },
    {
      ...
      handle: async function($, body) {
        ...
        let imgObj = await downloadFile(
          title,
          $("#sample-video a").attr("href"),
          {}
        );
      ...
      handle: async function(_, body) {
        ...
        if (res) {
          ...
          if (dmb) {
            ...
            //下载视频
            let videoFile = await downloadFile(
              this.title,
              "https:" + dmb.src,
              {}
            );
...
const addUser = async () => {
  ...
  //用户头像
  user.avatar =
    "https://pics.dmm.co.jp/mono/movie/adult/tkssni420/tkssni420pt.jpg";
  user.name = "fanza";
